    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    var userid =
                        localhostEntitiesDB.UCREDs.ToList()
                            .Where(u => u.EMAIL == model.UserName)
                            .Select(u => u.USERID)
                            .ToList();
                    Session["usersID"] = userid.FirstOrDefault();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var localhostEntitiesDb = new localhostEntities();
                localhostEntitiesDb.UCREDs.Add(ured);
                localhostEntitiesDb.SaveChanges();
                Session["usersID"] = ured.USERID;

                //string clientIp = (Request.ServerVariables["HTTP_X_FORWARDED_FOR"] ??
                //   Request.ServerVariables["REMOTE_ADDR"]).Split(',')[0].Trim();
                //
                var IPHost = Dns.GetHostName();
                var clientIp = Dns.GetHostByName(IPHost).AddressList[0].ToString();
                ;
                ...
                localhostEntitiesDb.UINFOes.Add(uinfo);
                localhostEntitiesDb.SaveChanges();
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("TopUploads", "Home");
                }
                AddErrors(result);
            }
